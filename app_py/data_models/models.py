...
class DevItemModel:
    ...
    def __post_init__(self):
        if self.vmx_path and not isinstance(self.vmx_path, Path):
            self.vmx_path = Path(self.vmx_path)

        if not self.name:
            self.name = self.vmx_path.name

        if self.vmx_path and not self.mac:
            kwargs = {}
    ...
    def __str__(self):
        return f'{self.name or "Undefined":<35} {self.mac or "Undefined":<20} {self.ip or "Undefined":<15}'
